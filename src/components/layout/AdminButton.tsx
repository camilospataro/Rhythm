...
export default function AdminButton({ impersonatingEmail }: AdminButtonProps) {
  ...
  if (impersonatingEmail) {
    return (
      <div className="flex items-center gap-1">
        <button
          onClick={handleStop}
          className="h-7 px-2 rounded-full bg-danger/15 border border-danger/30 flex items-center gap-1.5 text-danger hover:bg-danger/25 transition-colors"
          title={`Viewing as ${impersonatingEmail}`}
        >
          {isPending ? (
          ...
          <span className="text-[10px] font-semibold max-w-[80px] truncate">{impersonatingEmail}</span>
        </button>
      </div>
    );
  }
