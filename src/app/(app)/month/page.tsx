...
export default async function MonthPage({ searchParams }: MonthPageProps) {
  ...
  // Compute ISO weeks that overlap with this month (no DB needed)
  ...
  const [{ data: completions }, { data: tasks }, { data: assignments }, { data: allTemplates }, adminInfo] = await Promise.all([
    supabase
      .from("completions")
